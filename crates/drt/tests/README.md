...
| `cap3_crypto_jwt` | — | unblocked: `connectors/crypto` exists; the slice is not ported yet |
| `cap4_swarm` | — | the swarm and `sql` are both in; the slice's supervisor shape lands with `drt start` |
| `cap5_ports_daemon` | — | unblocked: `listen.rs` + `drt start` exist; the slice is not ported yet |
