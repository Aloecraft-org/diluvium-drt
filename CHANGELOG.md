...
- **A `windows` profile, and an unprefixed Windows binary.**
  `drt_windows_x86_64.exe` is `slim` plus `tunnel`, `relay` and
  `wireguard`: the three tokio-backed verbs that build for
  `x86_64-pc-windows-gnu` today, so a Windows box gets `drt tunnel`
  and a relay from a release binary rather than from a checkout.
  Named rather than left as `custom`, because a profile is what
  `requires.connectors` is admitted against and what the examples
  gate skips by; the connector set is `slim`'s, since the three are
  verbs and not connectors. `drt_windows_x86_64_slim.exe` continues
  as the distribution-size profile, as on every other platform, and
  `BUILDINFO.txt` gains `profile.windows.connectors` and
  `profile.windows.features`, read off the binary on a Windows
  runner. Both binaries are run there before they are uploaded:
  `--version`, a program, `buildinfo`, a WireGuard key pair and its
  public half derived again, and the examples gate. The tunnel and
  the relay are built and smoked there, not driven: the gate's
  tunnel and WireGuard examples still declare `needs_build: full`.
  What keeps the profile short of `full` is two facts, not a
  policy: `exec` is unix-only, and `stun`, `netcheck`, `turn` and
  `ssh` reach `aws-lc-sys` through russh's default crypto backend,
  whose windows-gnu build wants NASM on the cross leg
  (ego-transport#5 moves russh onto `ring`, already in that tree;
  the four join the profile when it lands, or earlier at the price
  of NASM on the cross leg).
