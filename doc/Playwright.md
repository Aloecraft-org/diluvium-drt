...
- **The deferred pump exists** (doc/Wasm.md M3, merged here 2026-09-03). `Dispatcher::route`
...
- **Chromium is already in CI.** The `browser` job installs node 22 and
  runs `npx playwright install --with-deps chromium`; Playwright sits
  under `crates/drt-web/browser-test` beside the suite that drives the
  browser tier. A browser plugin's test has the same ingredients.
...
| its in-flight integration: wire id to `(instance, tok)`, `max_inflight`, deadlines, the `wake` policy | ~1-2 days | `Pump`'s in-flight table, landed |
...
| an example (`18-browser`, against a page the example serves itself, so it needs no network), docs, `capabilities/list` entries | ~1-2 days | `examples/run-all.sh` |
...
- **Sessions outlive calls.** `sql` was the first connector with state
  ...
  closed when the instance dies, not at process end; `SwarmHost::released`
  is the fact that says so, and the plugin channel is where it reaches
