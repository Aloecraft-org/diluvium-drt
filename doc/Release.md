...
- **A `v*-dev.*` tag**, pushed or dispatched with `publish=true` → the dev
  ...
  (`drt_browser_access.js` and `.d.ts`), no changelog entry, always a prerelease, and the ten newest dev releases
...
cross-built binary nothing ran is what the smoke steps exist to prevent.
The `profile.windows.*` lines of `BUILDINFO.txt` are read off the binary
there and travel as `drt_windows.buildinfo.txt`, the way the browser
module's are read inside Chromium.
