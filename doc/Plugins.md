...
> host that had no nodes to own anything. `doc/Plan-0.7.0.md` §2.3 settles
...
`wasmtime -S inherit-network=y -S tcp=y`, and M6 (landed here) serves a
deployment's listener from wasmtime through
non-blocking `std::net` sockets stepped from the drive loop, with no
`wasi:io/poll` crate and no threads -- `examples/17-serving-http` is that
deployment, curl'd through the wasip2 gate in CI. The plugin channel is that
...
| a wasmtime test: the echo plugin as a loopback service, a `drt.wasm` calling it through the wasip2 gate CI already runs | ~1 day |
...
delete in M7 are gone, so a browser transport starts from the terminal
contract in `doc/Browser.md`: the module imports nothing but
wasm-bindgen's glue, and a JS function or Worker on the other side of
that boundary is the transport.
