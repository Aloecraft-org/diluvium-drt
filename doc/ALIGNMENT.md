...
A `dev` tag builds one native platform plus the browser module, and skips
the other cross-compilation, wasip2 and the slow suites. The browser
module is in because a browser client is built against a dev build long
before anything is a candidate. Everything else runs the full gate. Better than a dispatch
