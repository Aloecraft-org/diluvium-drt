...
|---|---|---|---|---|
| **status** | released: linux static x86_64, darwin arm64 and x86_64 (`doc/Release.md`) | not built; `full` blocked on cross-compiling `aws-lc-sys` through russh, `slim` unrehearsed | built on `claude/drt-wasm-port-planning-4ua6qk`, gated through the examples in CI (M1, M6) | built on the same branch, gated in Chromium (M4) |
...
| **connectors** | `full`: time, fs, crypto, sql, ssh, rest, ssmtp, exec, listen; `slim`: time, fs, crypto, listen | `slim`'s set expected; `sql` expected; the tokio-backed three depend on the `aws-lc-sys` build | `wasi`: time, fs, crypto, sql, listen | `web`: time, fs, crypto |
...
| **the drive loop** | blocks: sleep, `block_on` (until M3 merges) | same | ticks; the deferred pump parks a slow connector (M3) | ticks from `setTimeout`; nothing may block |
| **the gate** | fmt, clippy, both test profiles, the examples gate | none yet | the examples gate through wasmtime: 7 at M1, plus `08` and a served fetchpoint at M6 | the examples in Chromium, and the REPL parity transcript |
